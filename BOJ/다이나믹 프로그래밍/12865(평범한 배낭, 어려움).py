...
순방향 반복: 낮은 인덱스부터 높은 인덱스로 dp 배열을 업데이트하면, 이미 업데이트된 값이 다시 사용되어 동일한 아이템을 여러 번 사용하는 오류가 발생할 수 있다.
역순 반복: 높은 인덱스부터 낮은 인덱스로 dp 배열을 업데이트하면, 이미 업데이트된 값이 다시 사용되지 않으므로 각 아이템이 한 번만 사용되도록 보장할 수 있다.
